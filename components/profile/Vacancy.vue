    ...
    <div class="profile w-full profile-mobile-main relative mt-10">
      ...
      <template v-if="isScreenSmall">
        <div class="profile-mobile-wrapper">
          <div v-if="vacancy !== null" class="profile-mobile profile__image profile__image--vacancy">
          ...
          <div>
            ...
            <p class="profile-mobile profile__name profile__name--vacancy color-purple">
              {{ vacancy?.employer?.email }}
            </p>
          </div>
          <!-- РЕДАКТИРОВАТЬ -->
          <NuxtLink v-if="isInMyVacancies" class="cursor-pointer"
            :to="{ name: 'vacancies-edit-id', params: { id: vacancy?.uuid } }">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <mask id="mask0_1986_2674" style="mask-type: alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="24"
                height="24">
        ...
        <div v-if="vacancy?.required_experience" class="flex items-center gap-1 mt-1">
          <svg width="24" height="21" viewBox="0 0 24 21" fill="none" xmlns="http://www.w3.org/2000/svg">
            <mask id="mask0_1986_2718" style="mask-type: alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="24"
              height="21">
              <rect y="0.839844" width="24" height="20.16" fill="#D9D9D9" />
            </mask>
        ...
        <template v-if="userStore.access_token !== ''">
          <div class="w-full flex items-center flex-col mt-3 mb-3" v-if="userStore.user.role.current === 'applicant'">
            ...
            <template v-else>
              <template v-if="activeVacancyComment === vacancy?.uuid">
                <button @click="toggleVacancyComment('')" class="btn vacancy__btn">Отмена</button>
              </template>
              <button @click="toggleVacancyComment(vacancy.uuid)" v-else class="btn vacancy__btn">
              ...
              <div v-if="activeVacancyComment === vacancy?.uuid" class="vacancies-list__item-review">
                ...
                <button @click="vacancyResponse({ uuid: vacancy.uuid, message: message })"
          ...
          <div v-else>
            <div v-if="isInMyVacancies" class="profile__reviews-section">
              ...
              <div v-else class="profile__reviews mt-3">
                <div v-for="(candidat, index) in responsesByVacancy" :key="index" class="profile__reviews-item">
                  <a class="profile__reviews-item-head profile-reviews-head-mobile">
                    <div class="flex items-center">
                      <div class="profile__reviews-item-avatar">
                        <img v-if="candidat.applicant.image === 'image.png'" src="@/assets/images/profile/profile.svg"
                          alt="" />
                        <img v-else :src="`${$config.public.baseURL}/storage/users/${candidat.applicant.image}`" alt="" />
                      </div>
                      <div>
      ...
      <template v-else>
        <div class="profile__left">
          <div class="flex gap-3">
            <div class="profile__image profile__image--vacancy" v-if="vacancy !== null">
              <img :src="`${$config.public.baseURL}/storage/users/${vacancy.employer.image}`" alt="profile" />
            </div>
            <div class="w-full">
              <!-- Имя -->
              <div class="w-full flex justify-between">
                ...
                <!-- РЕДАКТИРОВАТЬ -->
                <NuxtLink v-if="isInMyVacancies" class="cursor-pointer" :to="{
          ...
          <!-- НЕОБХОДИМ ОПЫТ РАБОТЫ -->
          <div v-if="vacancy?.required_experience" class="flex items-center gap-1 mt-2">
            <svg width="24" height="21" viewBox="0 0 24 21" fill="none" xmlns="http://www.w3.org/2000/svg">
              <mask id="mask0_1986_2718" style="mask-type: alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="24"
                height="21">
                <rect y="0.839844" width="24" height="20.16" fill="#D9D9D9" />
        ...
        <div class="profile__right">
          ...
          <template v-if="userStore.access_token !== ''">
            <div class="w-full flex items-center mt-3 mb-3 gap-4" v-if="userStore.user.role.current === 'applicant'">
              <p class="vacancy__check" v-if="searchResponse(vacancy?.uuid)">
                Вы откликнулись на эту вакансию
              </p>
              <template v-else>
                    ...
                    <a class="profile__reviews-item-head profile-reviews-head-mobile">
                      <div class="flex items-center">
                        <div class="profile__reviews-item-avatar">
                          <img v-if="candidat.applicant.image === 'image.png'" src="@/assets/images/profile/profile.svg"
                            alt="" />
                          <img v-else :src="`${$config.public.baseURL}/storage/users/${candidat.applicant.image}`"
                            alt="" />
                        </div>
                        <div>
                          <p class="profile__reviews-item-name">
