...
const competitions = computed(() => {
  if (userStore?.user?.userData?.competitions) {
      ...
      <template v-if="isScreenSmall">
        <div class="profile-mobile-wrapper">
          <div class="profile-mobile profile__image">
            <img
              v-if="
                (userStore.user.image &&
                  userStore.user.image === 'image.png') ||
                !userStore.user.image
              "
            ...
            <img
              v-else
              :src="`${$config.public.baseURL}/storage/users/${userStore.user.image}`"
              alt=""
            />
          </div>
          <p class="profile-mobile profile__name">
            {{ userStore.user.userData.name }}
            {{ userStore.user.userData.surname }}
          </p>
          <NuxtLink to="/settings" class="mt-5">
            <svg
              width="20"
              height="20"
              viewBox="0 0 20 20"
