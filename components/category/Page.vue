      ...
      <template v-if="vacancies.length > 0">
        ...
        >
          ...
          <template v-if="userStore?.access_token">
            <template v-if="userStore?.user?.role?.current === 'applicant'">
...
onMounted(async () => {
  ...
  if (route.params.id) {
    console.log(
      vacanciesStore?.vacancies.filter(
        (vacancy) => vacancy.category.uuid === route.params.id
      )
    );
    vacancies.value = vacanciesStore?.vacancies.filter(
      (vacancy) => vacancy.category.uuid === route.params.id
    );
  }
});
...
watch(vacanciesStore.vacancies, (oldValue, newValue) => {
  if (vacanciesStore.vacancies) {
    vacancies.value = newValue.filter(
      (vacancy) => vacancy.category.uuid === route.params.id
    );
  }
});
